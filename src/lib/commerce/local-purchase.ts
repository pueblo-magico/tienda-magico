import type { FulfillmentMode } from "@/types/commerce";

export const LOCAL_COLLECTION = "local_collection" as const;
export const DELIVERY = "delivery" as const;

export type { FulfillmentMode } from "@/types/commerce";

const errorMessages = {
  en: "Choose a valid fulfillment mode before checkout.",
  es: "Elegí una modalidad de entrega válida antes de continuar con el pago.",
} as const;

export class FulfillmentModeError extends Error {
  constructor(locale = "es") {
    super(
      locale.toLowerCase().startsWith("en")
        ? errorMessages.en
        : errorMessages.es,
    );
    this.name = "FulfillmentModeError";
  }
}

const fulfillmentModes = new Set<FulfillmentMode>([LOCAL_COLLECTION, DELIVERY]);

export function parseFulfillmentMode(
  value: unknown,
  locale?: string,
): FulfillmentMode {
  if (
    typeof value === "string" &&
    fulfillmentModes.has(value as FulfillmentMode)
  ) {
    return value as FulfillmentMode;
  }

  throw new FulfillmentModeError(locale);
}

export function validateFulfillmentModeForCheckout(
  value: unknown,
  locale?: string,
): FulfillmentMode {
  return parseFulfillmentMode(value, locale);
}

export function localSaleIdempotencyKey(orderId: string): string {
  const normalizedOrderId = orderId.trim();
  if (!normalizedOrderId) {
    throw new Error(
      "An order ID is required for a local-sale idempotency key.",
    );
  }

  return `local-sale:${normalizedOrderId}`;
}
