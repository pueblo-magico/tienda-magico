...
async function customer(req: PayloadRequest) {
  ...
  const user = await req.payload.findByID({ collection: 'users', id: req.user.id, depth: 0, overrideAccess: true, req })
...
function endpoint(path: string, method: 'get' | 'post', handler: (req: PayloadRequest, id: number) => Promise<Response>): Endpoint {
  return { path: `/storefront-customer/${path}`, method, handler: async req => {
    try {
      if (method === 'post') {
        const origin = req.headers.get('origin')
        if (!origin || !req.payload.config.csrf.includes(origin)) return reply({ error: 'forbidden' }, 403)
      }
...
export const customerAccountEndpoints: Endpoint[] = [
  endpoint('session', 'get', async (req, id) => {
    const user = await req.payload.findByID({ collection: 'users', id, depth: 0, req, overrideAccess: true })
  ...
  endpoint('references', 'get', async (req, id) => {
    ...
    let page = 1
    while (true) {
      const orders = await req.payload.find({ collection: 'orders', where: { customer: { equals: id } }, depth: 0, limit: 100, page, overrideAccess: true, req })
  ...
  endpoint('link', 'post', async (req, id) => {
    ...
    if (!input || typeof input !== 'object' || !('references' in input) || !Array.isArray(input.references) || input.references.length > 20 || !input.references.every(value => typeof value === 'string' && /^[1-9]\d*::[a-f0-9]{40}$/.test(value))) return reply({ error: 'invalid' }, 400)
    ...
    try {
      ...
      for (const reference of [...new Set(input.references as string[])].sort()) {
        await transaction.execute(sql`SELECT pg_advisory_xact_lock(hashtextextended(${`cart:${reference}`}, 0))`)
