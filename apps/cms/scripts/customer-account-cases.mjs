...
export async function customerAccountCases({ payload, createLocalReq, user, product, order }) {
  ...
  const createCustomer = async email => payload.create({ collection: 'users', data: { email, password, roles: ['customer'], name: 'Cliente de prueba' } })
  const first = await createCustomer('first-account@example.test')
  const second = await createCustomer('second-account@example.test')
  const login = async email => (await payload.login({ collection: 'users', data: { email, password } })).token
  ...
  const request = async (path, token, body, origin = new URL(payload.config.serverURL).origin) => {
    ...
    req.headers = headers
    req.json = async () => body
    const endpoint = payload.config.endpoints.find(entry => entry.path === `/storefront-customer/${path}`)
  ...
  assert.equal((await request('link', firstToken, { references: [reference] }, 'https://evil.example')).status, 403)
  assert.equal((await request('link', firstToken, { references: [reference] })).status, 200)
  assert.equal((await payload.findByID({ collection: 'orders', id: guest.id, depth: 0 })).customer, first.id)
  const sibling = await order([item], { cartReference: reference })
  await request('link', secondToken, { references: [reference] })
  assert.equal((await payload.findByID({ collection: 'orders', id: sibling.id, depth: 0 })).customer ?? null, null)
  ...
  assert.equal((await payload.findByID({ collection: 'orders', id: sibling.id, depth: 0 })).customer, first.id)
  ...
  await payload.update({ collection: 'users', id: second.id, data: { sessions: [] }, req: adminReq })
